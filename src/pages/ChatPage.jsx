...
// ✅ Direct Change 1: Cleanly imported only uploadToCloudinary handler parameter explicitly
...
function ChatRoomPage({
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        // ✅ Direct Change 4: Replaced raw fetch block with unified uploadToCloudinary pipeline setup
        const upload = await uploadToCloudinary(blob, {
          resourceType: "video",
        });

        const audioUrl = upload.url;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // ✅ Direct Change 3: Refactored selectedImage uploader object extraction mapping urls cleanly
      if (selectedImage) {
        const upload = await uploadToCloudinary(selectedImage);
        imageUrl = upload.url;
      }

      // ✅ Direct Change 2: Refactored selectedFile uploader using auto route properties
      if (selectedFile) {
